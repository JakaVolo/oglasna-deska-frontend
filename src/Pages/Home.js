...
function Home() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://localhost/oglasna-deska-backend/get_post.php");
      setPosts(response.data);
      const uniqueCategories = [...new Set(response.data.map((post) => post.category_name))];
